import requests, time, re, json, os, datetime
...
def main():
    ...
    def get_time_in_timezone(hours_offset):
        # 获取当前 UTC 时间
        utc_now = datetime.datetime.utcnow()
        
        # 计算时区偏移
        tz_offset = datetime.timedelta(hours=hours_offset)
        
        # 应用时区偏移
        target_time = utc_now + tz_offset
        
        # 格式化时间为 YYYY-MM-DD 格式
        formatted_time = target_time.strftime('%Y-%m-%d')
    
        return formatted_time

    # 假设你要获取 UTC+8 时区的时间
    hours_offset = 8
    today = get_time_in_timezone(hours_offset)
...
if __name__ == "__main__":
    for i in range(len(eval(os.environ['USER_PHONE']))):
